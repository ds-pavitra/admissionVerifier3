...
const ViewApplication = ({ application }) => {
    ...
    const handleSubmit = async () => {
        if ((status === "rejected" || status === "pending" || status === "reverification") && reason.trim() === "") {
            setError("Reason is required for rejection, reverification or pending status.");
            return;
        }
        ...
        const payload = {
            ...
            status,
            reason: status === "approved" ? "" : reason, // If approved, no reason needed
        };

        try {
            const response = await apiRequestAsync("post", `${apiBaseUrl}/update-application-status`, payload);
            if (response.status === 200) {
    ...
    return (
        <div className="container">
            ...
            {(status === "pending" || status === "rejected" || status === "reverification") && (
